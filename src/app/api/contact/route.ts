  ...
  try {
    ...
    const emailResult1 = await resend.emails.send({
      from: process.env.NODE_ENV === 'production'
        ? 'Meicon LLC <noreply@mei-con.jp>'
        : 'Meicon Contact <onboarding@resend.dev>',
    ...
    const emailResult2 = await resend.emails.send({
      from: 'Meicon LLC <onboarding@resend.dev>', // Resendのテスト用ドメイン（本番環境では 'Meicon LLC <noreply@mei-con.jp>' に変更）
      to: [email],
